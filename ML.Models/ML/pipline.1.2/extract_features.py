...
def apply_bandpass_filter(data_array, fs=100, lowcut=0.5, highcut=8.0):
    nyquist = 0.5 * fs
...
def build_master_dataset(input_folder, output_filename):
    ...
    for file_path in all_files:
        ...
        clean_red = apply_bandpass_filter(raw_red)
        clean_ir = apply_bandpass_filter(raw_ir)
        ...
        # 3. Heart Rate (BPM)
        # distance=50 means peaks must be at least 0.5s apart (Max 120 BPM)
        peaks, _ = find_peaks(clean_red, distance=50)
        total_seconds = len(raw_red) / 100.0  # 100 Hz sampling rate
        heart_rate = (len(peaks) / total_seconds) * 60 if total_seconds > 0 else 0

        # 4. Waveform Morphology (Shape)
        ...
        # 5. Signal Power (Proxy for Pulse Area)
